  ...
  function mount(root,hooks){
      ...
      root.innerHTML='<main class="designer-page compact-designer old-model-designer unlimited-layer-designer">'+
        ...
        '<div class="designer-layout compact-designer-layout"><section class="designer-stage compact-stage locked-garment-stage" data-stage>'+
          ...
          '<aside class="designer-controls compact-controls"><div class="control-head"><span>DESIGN CONTROLS</span><small>'+surfaceLabel()+'</small></div>'+
            ...
            '<section class="control-block compact-choice-block"><div><label>Garment colour</label><div class="swatches">'+presetColours.map(c=>'<button aria-label="'+c+'" title="'+c+'" data-action="color" data-value="'+c+'" class="'+(state.color===c?'active':'')+'" style="background:'+S.palette[c]+'">'+(state.color===c?I('check'):'')+'</button>').join('')+'<button type="button" aria-label="Choose any colour" title="Choose any colour" data-action="custom-color-open" class="custom-colour-swatch '+(usingCustomColour()?'active':'')+'" style="--custom-colour:'+customColourValue()+'"></button><input type="color" class="visually-hidden-input" data-custom-garment-colour-input value="'+customColourValue()+'" aria-label="Choose any colour"></div></div><div><label>Size</label><div class="styled-select">'+picker('size',state.size,["XS","S","M","L","XL","XXL","3XL"].map(v=>({value:v,label:v})),state.pickerOpen)+'</div></div></section>'+
    ...
    function bind(){
      root.querySelectorAll('[data-action]').forEach(el=>el.addEventListener('click',e=>{
        ...
        else if(a==='custom-color-open'){root.querySelector('[data-custom-garment-colour-input]')?.click();}
      ...
      const customColorInput=root.querySelector('[data-custom-garment-colour-input]');if(customColorInput)customColorInput.addEventListener('input',()=>{state.color=customColorInput.value;if(S.isDarkColor(state.color)&&chosenPrint().lightOnly)state.printType=state.printMethods.find(p=>!p.lightOnly)?.name||'DTF Print';saveDraft();render();});
