(function(){
  ...
  const draftKey="one-line-designer-draft-v5";
  ...
  function mount(root,hooks){
    ...
    const state={
      ...
      surface:saved?.surface||"front",designs:saved?.designs||{front:blank(),back:blank(),rightSleeve:blank(),leftSleeve:blank()},
      ...
      root.innerHTML='<main class="designer-page compact-designer old-model-designer">'+
        ...
        '<div class="designer-layout compact-designer-layout"><section class="designer-stage compact-stage locked-garment-stage" data-stage>'+
          ...
          '<aside class="designer-controls compact-controls"><div class="control-head"><span>DESIGN CONTROLS</span><small>'+surfaceLabel()+'</small></div>'+
            ...
            (d.text?'<section class="control-block text-control active-layer-editor"><label>Text on '+surfaceLabel()+'</label><div class="input-with-icon">'+I('type')+'<input data-text value="'+S.esc(d.text)+'" maxlength="80" placeholder="Add text"><button data-action="remove" data-layer="text" aria-label="Clear text">'+I('close')+'</button></div><div class="inline-fields"><div class="font-field"><span>Font</span><div class="styled-select">'+picker('font',d.font,fontList.map(v=>({value:v,label:v})),state.pickerOpen)+'</div></div><input data-color-input type="color" value="'+d.textColor+'" aria-label="Text colour"><label class="range-field practical-size-range">Size <small>'+Math.round(d.textSize)+' px</small><input data-range="textSize" type="range" min="6" max="50" value="'+Math.min(TEXT_MAX,d.textSize)+'"></label><label class="range-field full-range">Rotate <small>'+Math.round(d.textRotation)+'°</small><input data-range="textRotation" type="range" min="-180" max="180" value="'+d.textRotation+'"></label></div></section>':'')+
            (d.uploadedImage?'<section class="control-block uploaded-control"><label>Image on '+surfaceLabel()+'</label><button class="refined-upload" data-action="choose-image"><img src="'+d.uploadedImage+'" alt=""><span><b>Change image</b><small>PNG, JPG or WEBP</small></span>'+I('upload')+'</button><div class="artwork-tools"><label class="practical-size-range">Size <small>'+Math.round(d.imageSize)+' px</small><input data-range="imageSize" type="range" min="10" max="'+maxSize+'" value="'+Math.min(maxSize,d.imageSize)+'"></label><label>Rotate<input data-range="imageRotation" type="range" min="-180" max="180" value="'+d.imageRotation+'"></label><button data-action="reset-image">'+I('rotate')+' Reset</button></div></section>':'')+
    ...
    function focusTextEditor(){state.active='text';render();requestAnimationFrame(()=>{const input=root.querySelector('[data-text]');if(!input)return;try{input.focus({preventScroll:true});}catch(_){input.focus();}input.select();});}
    ...
    function bind(){
      ...
      const txt=root.querySelector('[data-text]');if(txt)txt.addEventListener('input',()=>{current().text=txt.value;state.active=txt.value.trim()?'text':null;const textEl=root.querySelector('.layer-text-content');if(textEl)textEl.textContent=txt.value;saveDraft();if(!txt.value.trim())render();});
      ...
      root.querySelectorAll('[data-range]').forEach(el=>el.addEventListener('input',()=>{current()[el.dataset.range]=el.dataset.range==='textSize'?clampText(el.value):Number(el.value);const label=el.closest('label')?.querySelector('small');if(label){if(el.dataset.range==='textSize'||el.dataset.range==='imageSize')label.textContent=Math.round(Number(el.value))+' px';else if(el.dataset.range==='textRotation'||el.dataset.range==='imageRotation')label.textContent=Math.round(Number(el.value))+'°';}layerStyle(el.dataset.range.startsWith('text')?'text':'image');saveDraft();}));
      root.querySelectorAll('[data-layer]').forEach(el=>{
        ...
        el.addEventListener('pointerdown',e=>{
          ...
          state.active=layer;root.querySelectorAll('[data-layer]').forEach(node=>node.classList.toggle('selected',node===el));
          ...
          pointers.set(e.pointerId,{x:e.clientX,y:e.clientY,startX:e.clientX,startY:e.clientY,layer,offsetX:e.clientX-centerX,offsetY:e.clientY-centerY,moved:false});
      ...
      const stage=root.querySelector('[data-stage]');if(stage)stage.addEventListener('pointerdown',e=>{if(!e.target.closest('.design-layer')&&!e.target.closest('.custom-picker')&&state.active){state.active=null;root.querySelectorAll('[data-layer]').forEach(node=>node.classList.remove('selected'));}});
    }
    render();
