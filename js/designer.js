(function(){
  ...
  function mount(root,hooks){
    ...
    function render(){
      const d=current(),sleeve=state.surface.includes("Sleeve"),src=garmentSrc(),dark=S.isDarkColor(state.color),maxSize=resizeMax();
      root.innerHTML='<main class="designer-page compact-designer old-model-designer">'+
        ...
        '<div class="designer-layout compact-designer-layout"><section class="designer-stage compact-stage locked-garment-stage" data-stage>'+
          ...
          '<aside class="designer-controls compact-controls"><div class="control-head"><span>DESIGN CONTROLS</span><small>'+surfaceLabel()+'</small></div>'+
            '<section class="control-block garment-type-control"><label>Type</label><div class="garment-type-cards">'+models.map(m=>'<button data-action="model-card" data-value="'+S.esc(m.name)+'" class="'+(m.name===state.model.name?'active':'')+'"><b>'+S.esc(m.type)+'</b>'+(m.typeExtra?'<small>+'+S.money(m.typeExtra)+'</small>':'<small>Base type</small>')+'</button>').join('')+'</div></section>'+
    ...
    function bind(){
      ...
      root.querySelectorAll('[data-range]').forEach(el=>el.addEventListener('input',()=>{current()[el.dataset.range]=Number(el.value);const label=el.closest('label')?.querySelector('small');if(label&&(el.dataset.range==='textSize'||el.dataset.range==='imageSize'))label.textContent=Math.round(Number(el.value))+' px';layerStyle(el.dataset.range.startsWith('text')?'text':'image');saveDraft();}));
