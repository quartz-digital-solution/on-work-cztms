(function(){
  ...
  function mount(root,hooks){
    ...
    const state={
      model:models.find(m=>m.name===saved?.modelName)||models[0],color:saved?.color||"Navy",size:saved?.size||"M",qty:Number(saved?.qty)||1,
    ...
    surfaces.forEach(x=>{if(!state.designs[x.id])state.designs[x.id]=blank();});
    ...
    const saveDraft=()=>localStorage.setItem(draftKey,JSON.stringify({modelName:state.model.name,color:state.color,size:state.size,qty:state.qty,surface:state.surface,designs:state.designs,printType:state.printType}));
    ...
    function render(){
      const d=current(),sleeve=state.surface.includes("Sleeve"),src=garmentSrc(),dark=S.isDarkColor(state.color);
      root.innerHTML='<main class="designer-page compact-designer old-model-designer">'+
        ...
        '<div class="designer-layout compact-designer-layout"><section class="designer-stage compact-stage locked-garment-stage" data-stage>'+
          '<div class="compact-designer-bar"><div class="compact-surface-tabs" aria-label="Choose print area">'+surfaces.map(x=>'<button data-action="surface" data-value="'+x.id+'" class="'+(x.id===state.surface?'active':'')+'"><span class="full-label">'+x.label+'</span><span class="short-label">'+x.short+'</span>'+(state.designs[x.id].text||state.designs[x.id].uploadedImage?'<i></i>':'')+'</button>').join('')+'</div>'+
          '<div class="apparel-dropdown cloth-type-dropdown"><span>Cloth type</span>'+picker('model',state.model.name,modelOptions,state.pickerOpen,'model-picker')+'</div></div>'+
          ...
          '<aside class="designer-controls compact-controls"><div class="control-head"><span>DESIGN CONTROLS</span><small>'+surfaceLabel()+'</small></div>'+
            '<section class="control-block cloth-type-control"><label>Cloth type</label><div class="cloth-type-cards">'+models.map(m=>'<button data-action="model-card" data-value="'+S.esc(m.name)+'" class="'+(m.name===state.model.name?'active':'')+'"><b>'+S.esc(m.type)+'</b><small>'+S.money(m.base)+'</small></button>').join('')+'</div></section>'+
            ...
            '<section class="control-block text-control"><label>Text on '+surfaceLabel()+'</label><div class="input-with-icon">'+I('type')+'<input data-text value="'+S.esc(d.text)+'" maxlength="80" placeholder="Add text"><button data-action="remove" data-layer="text" aria-label="Clear text">'+I('close')+'</button></div><small class="edit-tip">Double tap the text on the shirt anytime to edit it.</small><div class="inline-fields"><div class="font-field"><span>Font style</span><div class="styled-select">'+picker('font',d.font,fontList.map(v=>({value:v,label:v})),state.pickerOpen)+'</div></div><input data-color-input type="color" value="'+d.textColor+'" aria-label="Text colour"><label class="range-field">Size<input data-range="textSize" type="range" min="6" max="1200" value="'+Math.min(1200,d.textSize)+'"></label><label class="range-field full-range">Rotate<input data-range="textRotation" type="range" min="-180" max="180" value="'+d.textRotation+'"></label></div></section>'+
            (d.uploadedImage?'<section class="control-block uploaded-control"><label>Image on '+surfaceLabel()+'</label><button class="refined-upload" data-action="choose-image"><img src="'+d.uploadedImage+'" alt=""><span><b>Replace image</b><small>PNG, JPG or WEBP</small></span>'+I('upload')+'</button><div class="artwork-tools"><label>Size<input data-range="imageSize" type="range" min="10" max="1400" value="'+Math.min(1400,d.imageSize)+'"></label><label>Rotate<input data-range="imageRotation" type="range" min="-180" max="180" value="'+d.imageRotation+'"></label><button data-action="reset-image">'+I('rotate')+' Reset</button></div></section>':'')+
    ...
    function add(){const customDesign=makeDesign();hooks.onAdd({key:'custom-'+Date.now(),name:state.model.name+' · Custom',price:unitPrice(),qty:state.qty,color:state.color,size:state.size,detail:state.printType+' · '+designedCount()+' print area'+(designedCount()===1?'':'s'),image:state.model.image,custom:true,customDesign});saveDraft();}
    ...
    function bind(){
      root.querySelectorAll('[data-action]').forEach(el=>el.addEventListener('click',e=>{
        ...
        else if(a==='pick'){const n=el.dataset.pickerName,v=el.dataset.value;if(n==='model')chooseModel(v);if(n==='size')state.size=v;if(n==='font')current().font=v;state.pickerOpen=null;saveDraft();render();}
      ...
      root.querySelectorAll('[data-range]').forEach(el=>el.addEventListener('input',()=>{current()[el.dataset.range]=Number(el.value);layerStyle(el.dataset.range.startsWith('text')?'text':'image');saveDraft();}));
      ...
      root.querySelectorAll('[data-transform]').forEach(handle=>{
        ...
        handle.addEventListener('pointermove',e=>{const g=transformGesture;if(!g||g.id!==e.pointerId)return;e.preventDefault();e.stopPropagation();const angle=Math.atan2(e.clientY-g.cy,e.clientX-g.cx)*180/Math.PI,distance=Math.max(1,Math.hypot(e.clientX-g.cx,e.clientY-g.cy));let rotation=g.rotation+angle-g.angle;if(Math.abs(rotation%45)<3)rotation=Math.round(rotation/45)*45;const size=g.size*(distance/g.distance);if(g.layer==='text'){current().textRotation=Math.round(rotation);current().textSize=clampMin(size,6);}else{current().imageRotation=Math.round(rotation);current().imageSize=clampMin(size,10);}layerStyle(g.layer);});
    ...
    render();
    return{destroy(){syncCurrentScale();root.innerHTML='';}};
  }
