(function(){
  ...
  function bindFastUniformCustomizer(){
    ...
    const open=()=>{syncHex(current);if(hex)hex.value=current.slice(1).toUpperCase();overlay.classList.add('open');overlay.setAttribute('aria-hidden','false');document.documentElement.classList.add('fast-picker-lock');document.body.classList.add('fast-picker-lock');};
    const close=()=>{overlay.classList.remove('open');overlay.setAttribute('aria-hidden','true');document.documentElement.classList.remove('fast-picker-lock');document.body.classList.remove('fast-picker-lock');};
