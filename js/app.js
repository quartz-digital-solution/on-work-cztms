(function(){
  ...
  function product(){
    ...
    const label=String(p.name||p.subcategory||p.category||'Product').trim()||'Product',hasPrice=Number(price)>0||Number(p.mrp)>0;
    ...
    const title=p.name?'<h1>'+S.esc(p.name)+'</h1>':'';
    const priceBlock=hasPrice?'<div class="detail-price"><strong>'+S.money(price)+'</strong>'+(p.mrp?'<s>'+S.money(p.mrp)+'</s>':'')+'</div>':'';
    const desc=p.description?'<p>'+S.esc(p.description)+'</p>':'';
    const addButton=Number(price)>0?'<button class="primary detail-add" data-action="add-product">Add to cart '+I('bag')+'</button>':'';
    return '<main class="product-fullscreen screen screen-enter"><div class="product-full-back"><button class="back-button" data-action="nav-back" data-fallback="catalog">'+I('back')+' Back</button></div><div class="product-full-inner"><section class="product-full-media">'+detailSlider(gallery,label)+'</section><section class="product-full-info"><div class="product-title-tools"><span class="eyebrow">'+[p.category,p.subcategory].filter(Boolean).map(S.esc).join(' / ')+'</span><button type="button" class="inline-share-button" data-share-kind="product" data-share-value="'+S.esc(p.id)+'" data-share-label="'+S.esc(label)+'" aria-label="Share product">'+I('share')+'</button></div>'+title+priceBlock+desc+(p.colors?.length?'<div class="selection-block"><label>Colour <b>'+S.esc(state.color)+'</b></label><div class="colour-options">'+p.colors.map(c=>'<button class="'+(state.color===c?'active':'')+'" data-product-color="'+S.esc(c)+'"><span style="background:'+(S.palette[c]||'#ddd')+'"></span>'+S.esc(c)+'</button>').join('')+'</div></div>':'')+(sizes.length?'<div class="selection-block"><label>'+S.esc(p.optionTitle||'Size')+' <b>'+S.esc(state.size)+'</b></label><div class="size-options">'+sizes.map(v=>'<button class="'+(state.size===v?'active':'')+'" data-product-size="'+S.esc(v)+'">'+S.esc(v)+'</button>').join('')+'</div></div>':'')+subPanel+'<div class="product-action-row">'+addButton+'<button class="custom-enquiry-btn" data-action="whatsapp" data-message="Hi, I want to know about '+S.esc(label)+'. Please share the details and customization options.">'+whatsappLogo()+'<span>Contact to customize</span></button></div>'+(p.name||p.description||hasPrice?'<div class="ready-made-note"><b>Ready-made item</b><span>This product itself is not edited in the customizer. Contact us for a custom version.</span></div>':'')+'</section></div></main>';
  }
  ...
  function bindFastUniformCustomizer(){
    ...
    const nativePicker=box.querySelector('[data-fast-uniform-native]'),dot=box.querySelector('[data-fast-uniform-dot]');
    ...
    let original=null,shirtMask=null,current='#20f12e',imgReady=false,maskReady=false;
    ...
    const setColour=(value,custom=false)=>{if(!/^#[0-9a-f]{6}$/i.test(value))return;current=value.toLowerCase();if(nativePicker)nativePicker.value=current;if(dot)dot.style.background=current;box.querySelectorAll('[data-fast-uniform-colour]').forEach(b=>b.classList.toggle('active',b.dataset.fastUniformColour.toLowerCase()===current));const hasPreset=[...box.querySelectorAll('[data-fast-uniform-colour]')].some(b=>b.dataset.fastUniformColour.toLowerCase()===current);const customButton=box.querySelector('.fast-uniform-custom-swatch');if(customButton){customButton.classList.toggle('active',custom||!hasPreset);customButton.style.setProperty('--fast-custom-colour',current);}drawColour(current);};
    ...
    box.querySelectorAll('[data-fast-uniform-native-open]').forEach(b=>b.addEventListener('click',()=>nativePicker?.click()));
    box.querySelector('[data-fast-uniform-reset]')?.addEventListener('click',()=>setColour('#20f12e',false));
    nativePicker?.addEventListener('input',e=>setColour(e.target.value,true));
  }
  function bind(){
