(function(){
  ...
  function shareUrl(kind,value,extra){
    const url=new URL(location.href);url.searchParams.delete('product');url.searchParams.delete('category');url.searchParams.delete('subcategory');url.searchParams.delete('audience');url.searchParams.delete('section');
  ...
  if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js').catch(()=>{});render();if(state.sharedSection==='shirt-colour'){setTimeout(()=>document.querySelector('[data-fast-uniform]')?.scrollIntoView({behavior:'auto',block:'start'}),120);}
})();
